...
## 基準測試案例(回歸複查用,對應 spec 第49.4節)

`sql/golden_test_cases.md` 記錄幾個合成測試案件(不含真實客戶資料)已核對正確的關鍵金額(各段Subtotal/Total、What-if情境重量結果)。修改牽涉**兩個以上檔案**、動到**資料庫欄位**、或觸及**FeeLine/Segment 核心計算邏輯**(沿用第20節既有的「大範圍改動」門檻)時,除了測試新行為,也應該對照這份文件核對有沒有把既有功能改壞——優先讀程式碼手動核算比對,對不上才需要進一步用瀏覽器實測。
