...
3. 計算公式：`該項目在此級距的每KG貢獻 = (該級距估計數量的最大值 × 單位費率 [×天數，若是PerDay類型]) ÷ (該級距估計數量的最大值 × 每單位換算重量)`——例如 `5 PLT × 15 USD ÷ (5 × 250KG) = 75 ÷ 1250 = 0.06 USD/KG`
...
### 47.1 「可能成本」互斥選項組（Cost Option Groups，全新概念）

手算範例裡，`possible`成本被分成兩組互斥方案——「一般倉」跟「保稅倉」，是**兩選一**，不是都要算進去。目前系統的`possible`只能整批「要不要算」，沒有「這幾筆是一組互斥選項」的概念。

**新增設計**：
```
FeeLine.optionGroup   // 選填，字串標籤（如"一般倉"/"保稅倉"），null代表不屬於任何互斥組，正常依certain/possible規則計入
```
- 屬於同一個`optionGroup`的`possible`費用是一組互斥方案，比較分析/報價頁面要提供**單選**（不是複選）介面讓使用者選擇要計入哪一組（或都不計入），比照手算範例的「All-in with 一般倉」vs「All-in with 保稅倉」分開呈現、可比較
- 沒有`optionGroup`（null）的`possible`費用維持原本行為（獨立判斷要不要計入，不受這個機制影響）
- 這個機制要能跟What-if情境分析（第43/45節）搭配：同一組情境重量下，可以同時看到「一般倉」跟「保稅倉」兩條線各自的每KG成本，方便直接比較
...
> 1. **47.1 成本互斥選項組**：FeeLine新增`optionGroup`欄位，同組的possible費用是互斥方案（如一般倉/保稅倉），比較分析/報價頁要提供單選介面，What-if情境分析要能同時呈現不同選項組各自的每KG成本供比較
...
| `perPallet`/`perCarton` | 依47.2節：`amount × (bracketFloor ÷ 每單位換算重量)` | = `amount ÷ 每單位換算重量`（固定比例，恆定，如手算範例的Warehouse in&out） | 不會（除非切換成47.2節的進階逐級距模式） |
